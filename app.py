...
def load_data(uploaded_file):
    ...
    try:
        ...
        # 使用确定性随机数 (基于 GeneName)，避免每次刷新页面位置跳动
        def assign_fallback_pos(row):
            if row['Is_Valid_Chr'] and pd.isna(row['Pos_Mb']):
                # 使用 GeneName 和 Chr 组合作为种子，确保位置固定
                seed_source = str(row['GeneName']) + str(row['Chr_Clean'])
                # 使用 random.Random(seed) 创建局部随机生成器，不影响全局状态
                rng = random.Random(seed_source)
                return rng.uniform(46, 49)
            return row['Pos_Mb']
